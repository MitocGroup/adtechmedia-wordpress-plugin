...
const widgetApplied = 'Widget can be applied on posts for RO';

fixture `Widget can be applied on posts and modified from WP configuration page for RO`
...
test(widgetApplied, async t => {
  ...
  await t
    ...
    .typeText(elements.price,'0.02', { replace: true })
    .click(elements.save);
    ...
    .expect(widget.priceCheck.innerText).eql('0.02 RON')
    .expect(widget.payPledgeBtn.innerText).eql(' VOI PLATI')
    .expect(widget.subscribeBtn.innerText).eql(' ABONARE')
    ...
    .typeText(elements.price,'22', { replace: true })
    .click(elements.save);
    ...
    .expect(widget.widgetPlugin.visible).ok()
    .click(widget.expandBtn)
    .expect(widget.priceCheck.innerText).eql('€22')
    .expect(widget.payPledgeBtn.innerText).eql(' VOI PLATI')
    .expect(widget.showADBtn.innerText).eql(' PUBLICITATE');
    ...
    .expect(widget.priceCheck.innerText).eql('Monthly')
    .expect(widget.showADBtn.innerText).eql(' PUBLICITATE')
    .expect(widget.subscribeBtn1.innerText).eql(' ABONARE');
