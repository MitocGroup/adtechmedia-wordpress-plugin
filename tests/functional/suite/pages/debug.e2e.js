...
const widgetApplied = 'Widget can be applied on posts';

fixture `Widget can be applied on posts and modified from WP configuration page`
...
test(widgetApplied, async t => {
  ...
  await t
    ...
    .expect(widget.priceCheck.innerText).eql('$1.23')
    ...
    .typeText(elements.price,'3.21', { replace: true })
    .click(elements.save);
    ...
    .expect(widget.widgetPlugin.visible).ok()
    .click(widget.expandBtn)
    .expect(widget.priceCheck.innerText).eql('$3.21')
    .expect(widget.payPledgeBtn.innerText).eql(' PLEDGE')
    ...
    // .typeText(elements.price,'0.11', { replace: true })
    // .click(elements.save);
